...
Page({
  ...
  onLoad(options) {
    ...
    try { interest = decodeURIComponent(interest); } catch (e) { /* keep raw value */ }
    this.setData({ type: options.type || '电动车', interest });
  },
  ...
  submit() {
    ...
    wx.request({
      url: `${API_BASE_URL}/api/leads`, method: 'POST',
      data: { userId: 'miniapp_guest', name: name.trim(), phone: phone.trim(), businessType: this.data.type, interest: this.data.interest || '待沟通', expectedTime: this.data.time, deliveryNeed: this.data.type === '电动车' ? '校内配送' : '无', note: this.data.note },
      success: (response) => {
